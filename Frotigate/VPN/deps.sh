...
for i in /usr/bin/mktemp /usr/bin/cat /usr/bin/grep /usr/bin/cut /usr/bin/date /usr/bin/ldapsearch /bin/rm /usr/bin/sort /usr/bin/uniq /usr/bin/echo /usr/bin/head /usr/bin/tail /usr/bin/tr ; do
	if [ -f $i ] ; then
		echo "$i ok"
	else
